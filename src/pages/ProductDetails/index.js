...
function ProductDetails() { 
    ...
    useEffect(() => {
        ...
        // On load, if there is no recently clicked product info then redirect to catalog
        if (localStorage.getItem("recently_clicked") === null) history.push('/catalog')
        else setIsLoading(false)
    }, [])

    useEffect(  () => {
        if (!isLoading) {
            getSimilarProducts(product["product_id"]).then(simProducts => {
